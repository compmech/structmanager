...
class PanelComp(SE2D):
    ...
    def __init__(self, name, eids, model=None):
        super(Panel, self).__init__(name, eids, model) #change to super(PanelComp, self)?
        ...
        self.p90 = 0.1
        # material properties
        self.E1 = #get from FE Model?
        self.E2 = #get from FE Model?
        self.G12 = #get from FE Model?
        self.nu12 = #get from FE Model?
    ...
    def create_dvars(self):
        ...
        if ptype == 'PCOMP':
            ...
            dvprel2.dtable = [self.p90]
            self.add_dvprel(dvprel2)

            #DONE
            deqatn45 = DEQATN('T45(t,p45) = p45*t')
            dvprel2 = DVPREL2('PCOMP', pid=pid, pname='T2', deqatn45.id)
            dvprel2.dvars = [dvar_t.id, dvar_p45.id]
            self.add_dvprel(dvprel2)

            #DONE
            deqatn90 = DEQATN('T90(t,p90) = p90*t')
            dvprel2 = DVPREL2('PCOMP', pid=pid, pname='T3', deqatn90.id)
            ...
            self.add_dtable('PCnu12', self.nu12)
            self.add_dtable('PCnu21', self.nu21)
        else:
    ...
    def constrain_buckling(self, eig=1.0):
        ...
        dresp.add_dvar(self.dvars['PCt0'].id)
        dresp.add_dvar(self.dvars['PCt45'].id)
        dresp.add_dvar(self.dvars['PCt90'].id)
        ...
        dresp.add_dtable(self.dtables['PCnu12'][0])
        dresp.add_dtable(self.dtables['PCnu21'][0])
        dresp.add_dresp2(dresp_Nxx.id)
        dresp.add_dresp2(dresp_Nyy.id)
        dresp.add_dresp2(dresp_Nxy.id)
        self.add_dresp(dresp)
        # applying constraint of buckling: lower eigenvalue >= 1.0
        self.add_constraint(dcid, dresp3, eig)
