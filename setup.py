#!/usr/bin/env python
"""StructManager: Structural Manager for Python
...
def write_version_py(filename='structmanager/version.py'):
    cnt = """
# THIS FILE IS GENERATED FROM StructManager setup.py
...
def configuration(parent_package='', top_path=None):
    ...
    config.add_subpackage('structmanager')
    config.add_data_files(('structmanager', '*.txt'))

    config.get_version('structmanager/version.py')
...
def setup_package():
    ...
    build_requires = ['numpy>=1.10.4', 'scipy>=0.15.0']

    FULLVERSION, GIT_REVISION = get_version_info()

    metadata = dict(
        name='structmanager',
        maintainer='StructManager Developers',
        maintainer_email='saullo.castro@embraer.com.br ',
        version=FULLVERSION,
        description=DOCLINES[0],
        long_description='\n'.join(DOCLINES[2:]),
        url='http://structmanager.github.io/structmanager/',
        download_url='https://github.com/structmanager/structmanager/releases',
        license='BSD',
        cmdclass=cmdclass,
